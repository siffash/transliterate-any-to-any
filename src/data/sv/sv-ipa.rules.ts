export const svIpaRules = `
::Lower;
::NFC;

kristian > krɪɧan;
tion > ɧuːn;
sch > ɧ;
stj > ɧ;
skj > ɧ;
sj > ɧ;
sh > ɧ;
dj > j;
gj > j;
hj > j;
lj > j;
tj > ɕ;
kj > ɕ;
th > t;
ch > ɕ;

rt > ʈ;
rd > ɖ;
rn > ɳ;
rl > ɭ;
rs > ʂ;

ng > ŋ;
enkø > ɛnɕø;
nk > ŋk;
gn > ŋn;

sk } [eiyäö] > ɧ;
falk > falk;
k } [eiyäö] > ɕ;
g } [eiyäö] > j;
c } [eiyäö] > s;

rg } [^[:L:][:M:]] > rj;
lg } [^[:L:][:M:]] > lj;

ck > k;
qu > kv;
q > k;
w > v;
ex > ɛks;
x > ks;
z > s;
c > k;

ese } [^[:L:][:M:]] > eːs;
nne } [^[:L:][:M:]] > n;
ice } [^[:L:][:M:]] > ɪs;
au > aʊ;
oy > ɔj;
a } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] > a;
e } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] > ɛ;
i } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] > ɪ;
oo > uː;
o } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] > ɔ;
u } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] > ɵ;
y } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] > ʏ;
å } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] > ɔ;
ei > ɛj;
ä } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] > ɛ;
ö } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] > œ;

a } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [^[:L:][:M:]] > a;
e } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [^[:L:][:M:]] > ɛ;
i } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [^[:L:][:M:]] > ɪ;
o } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [^[:L:][:M:]] > ɔ;
u } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [^[:L:][:M:]] > ɵ;
y } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [^[:L:][:M:]] > ʏ;
å } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [^[:L:][:M:]] > ɔ;
ä } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [^[:L:][:M:]] > ɛ;
ö } [bcdfghjklmnpqrstvwxzʈɖɳɭʂɧɕŋ] [^[:L:][:M:]] > œ;
a } [^[:L:][:M:]] > a;

a > ɑː;
e > eː;
i > iː;
o > uː;
u > ʉː;
y > yː;
å > oː;
ä > ɛː;
ö > øː;

bb > b;
dd > d;
ff > f;
gg > ɡ;
kk > k;
ll > l;
mm > m;
nn > n;
pp > p;
rr > r;
ss > s;
tt > t;
vv > v;

b > b;
d > d;
f > f;
g > ɡ;
hl > l;
h > h;
j > j;
k > k;
l > l;
m > m;
n > n;
p > p;
r > r;
s > s;
t > t;
v > v;
`;
